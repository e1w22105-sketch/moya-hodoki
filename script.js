...
const WHEN_LIST = {
    ...
    level2: [
        ...
        "窓を全開にした時に", "お湯が沸騰した瞬間に", "カレンダーをめくった瞬間に", "パンが焼けた良い匂いの中で", "深夜の静まり返った部屋で",
    ...
    level3: [
        ...
        "信号が赤に変わった瞬間に", "世界が止まったような気がして", "あわよくばの精神で", "懐かしい曲が流れてきた時に", "前ぶれもなく唐突に",
...
const ENDINGS = [
    "少し軽くなったなら、それで十分。","ここにきた時点で、もう半分モヤモヤは外に出たよね","今日はここまでにしよ","また溜まったら、置きに来て。",
    "今日は、空気がちょっとだけ濁ってただけだよ。","たぶん世界が少し寝ぼけてただけだから、あなたのせいじゃないよ。","今日の運勢、三年前の消しゴムが決めたらしいから言っとくね",
    "道があなたに優しくない日って、たまにあるよね。","小さなズレは、明日のおもしろポイントらしいよ。","今日のあなた、ちょっと風に振り回されてただけだと思う。",
    "無理に元気出さなくて大丈夫。靴だって迷う日があるしね。","気配がざわつく日は、早く帰ってお茶しな","すれ違った人たち、みんなボタンを掛け違えてたらしいよ。そんな日。",
    "世界がほんの少し揺れてただけ。あなたはいつもどおり。","うまくいかない日は、何しても可愛いよ。","今日は頭があなたに厳しかっただけ。",
    "思ったより大変だったね。イスもたぶん応援してたよ。","風の機嫌が悪くて、あなたの髪だけ狙われてたらしい。","今日のほどきは、明日のあなたに返してあげるってさ。",
    "あなたの今日の不調、たぶん天気が勝手に決めたんだよ","ズレてたのは世界のほう。あなたはよくやったよ。",
    "今日のあなた、静かにがんばってて好きだよ。","ちょっとだけ、気持ちが追いつかない時間だったね。","無理しないでいいよ。ペンですら休みたがってた日だし。",
    "モヤは、世界があなたに“ゆっくりしていいよ”って言ってる合図。","あれこれ考えなくても大丈夫。歩幅が戻れば全部うまくいくよ。",
    ...
    "さっきのモヤ、たぶん昨日すれ違った黒猫が運気を吸い取ったせいだわ。","今日のモヤ、昨日の夜に誰かが置いていった忘れ物だよ。","太陽が少しだけ意地悪だっただけ。次はいい位置にくるよ。",
    "今日の不運は、来週のあなたが笑うための前振りらしいよ。","大丈夫。そのうち風がどっかに運んでくれるから、忘れていいよ。","今日はもう、十分すぎるくらい頑張った。あとは寝るだけ。",
    "世界があなたに甘えたがってる日なんだよ。","たぶん今のモヤモヤ、昨日座っていた椅子のせいだよ。知らんけど。","あなたのせいじゃない。太陽がちょっと乱れてただけ。",
    "明日は軽くなるよ。","明日のあなたはモヤを笑い飛ばしてるよ。","大丈夫、世界はあなたの味方をするのを一瞬忘れただけ。","よし。今日はもう、自分を甘やかして終わろうね。"
            ...
            <div id="resultContent" class="result-container" style="display:none;">
                ...
                <div class="result-actions">
                    <div class="button-group">
                        ...
                        <button class="primary-btn" onclick="go('diary')">ほどけた</button>
                    </div>
                </div>
            </div>
        `;
    ...
    // --------------------------------------------------
        ...
        container.innerHTML = `
            <div class="background-layer">
                <div class="char-container result-char">
                    <div class="speech-bubble show">よし、終わり</div>
                    <div class="char-visual">
                        <img src="assets/moyataro2.svg" class="floating">
                    </div>
                </div>

                <div class="thought-bubble-result ${resultData.levelClass} show">
                    <div class="bubble-content-center">${resultData.text}</div>
                    <button class="fav-btn-stamp">
                          ${isFav ? "★" : "☆"}
                    </button>
                </div>
            </div>
            ...
            <div class="overlay-layer">
                <div class="sticker-wrapper">
                    <div class="message-sticker">
                        <p class="message-text">${getRandElement(ENDINGS)}</p>
                        <div class="message-signature">モヤたろうより</div>
                    </div>
                </div>

                <button onclick="finishAndHome()" class="fade-in-btn primary-btn" style="margin-top: 30px;">ホーム</button>
            </div>
        `;
    }
}
...
function saveAndGoToDiary() {
    ...
    const inputElement = document.getElementById("moyaInput"); // 入力欄IDを確認してください(後述)
    
    // input要素が見つからない場合の保険（HTMLの構造による）
    const inputVal = inputElement ? inputElement.value : (state.input.who + "が" + state.input.where + state.input.particle + state.input.what);
    ...
    state.diary.unshift({
        ...
        levelClass: currentResult.levelClass, 
        time: new Date().toLocaleDateString(),
        original: inputVal,     // 原文保存
        isFavorite: true        // ★ここをtrueに変更（自動お気に入り）！
    });
...
function transform() {
    ...
    // 配列を結合。何も入力がない場合は「不明なモヤモヤ」などにする
